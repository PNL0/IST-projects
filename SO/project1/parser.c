...
int check_write(char *output_filename, int fd_out, char* buffer, size_t totalBytesToWrite){
  size_t bytesWritten = 0;
  while(bytesWritten < totalBytesToWrite){
    ssize_t bytesWritten_aux = write(fd_out, buffer + bytesWritten, totalBytesToWrite - bytesWritten);
    if(bytesWritten_aux == -1) {
      fprintf(stderr, "Error writting the file called %s.\n", output_filename);
      return 0;
    }
    else {
